...
라우터는 네트워크의 모든 호스트가 유지하는 **라우터 테이블(routing table)**에 저장된 정보를 이용하여 경로를 선택한다. 라우팅 테이블의 엔트리는 목적지 호스트와 다음 홉을 포함한다. 목적지 호스트에는 패킷의 최종 목적지 호스트의 주소 값을 저장한다. 다음 홉에는 목적지 호스트로 가기 위한 최적의 이웃 라우터를 지정한다.

#### 홉이란

**홉(hop)**은 목적지에 도달할 때까지 거치는 게이트웨이의 개수를 이른다.
...
IP 주소는 네트워크 상에서 고유하게 호스트를 구분하는 주소이다. 호스트가 인터넷에 접속하려면 반드시 IP 주소를 할당받아야한다. 
...
**서브넷(subnet)**은 하나의 네트워크에서 분할된 네트워크이다. 서브넷 마스크를 사용하여 네트워크를 분할할 수 있으며 이를 서브네팅(subnetting)이라고 한다. IPv4 주소 부족 문제를 해결하기 위해 등장하였다.

서브넷에서 IP 주소는 네트워크 주소와 호스트 주소로 구분된다. 이때 네트워크 주소와 호스트 주소를 구분하기 위해 **서브넷 마스크(subnet mask)**를 사용하는 것이다. `0` 비트는 호스트, `1` 비트는 네트워크이다. 예를 들어보자.

```
IP주소: 192.168.123.132
서브넷 마스크: 255.255.255.0
```

이때 네트워크 주소는 `192.168.123`, 호스트 주소는 `0.0.0.132`가 된다. 서브넷이 `192.168.123`, 서브넷 내에서 호스트가 `192.168.123.132`이다.

### IP 주소 클래스

IP 주소는 네트워크 대역에 따라 다섯 개의 네트워크 클래스 A, B, C, D, E로 나뉜다. A, B, C는 유니캐스팅, D는 멀티캐스팅(일대다전송)에 사용한다. E는 새로운 응용 환경을 위해 잠정적으로 예약되었다.
...
| 클래스 | 사용하는 서브넷 마스크                           | 네트워크 주소 prefix | 네트워크 규모 | 네트워크 주소 개수                                   | 호스트 주소 개수 |
| ------ | ------------------------------------------------ | -------------------- | ------------- | ---------------------------------------------------- | ---------------- |
| A      | `255.0.0.0`<br />(네트워크 ID가 8비트 사용)      | `0`으로 시작         | 대규모        | 2^(8-1)-2 (왜냐하면 `0`번과 `127`번이 예약되어있다.) | 2^24-2           |
| B      | `255.255.0.0`<br />(네트워크 ID가 16비트 사용)   | `10`으로 시작        | 중간          | 2^(16-2)                                             | 2^16-2           |
| C      | `255.255.255.0`<br />(네트워크 ID가 24비트 사용) | `110`으로 시작       | 소규모        | 2^(24-3)                                             | 2^8-2            |

각 클래스의 호스트 주소 개수에서 2개를 빼는 이유는 다음과 같다.

- 호스트 주소를 표현하는 모든 비트의 값이 0이면 해당 서브넷 자체를 지칭한다.
- 호스트 주소를 표현하는 모든 비트의 값이 1이면 서브넷에 존재하는 모든 호스트를 지칭한다. (브로드캐스팅 용도로 사용한다.)

#### A 클래스의 0번과 127번

A 클래스의 서브넷 마스크는 보통 `255.0.0.0`이므로 이론상 사용가능한 IP 주소 범위는 0.xxx.xxx.xxx부터 127.xxx.xxx.xxx이다. 그런데 실제로는 1.xxx.xxx.xxx부터 126.xxx.xxx.xxx까지 사용가능하다. 

- `127`번: 루프백(loopback)을 지원하기 위해 예약되어있다. 컴퓨터는 127번을 사용하여 자체적으로 통신한다. 일반적으로 호스트가 자기 자신(로컬 호스트)을 가리키기 위해 `127.0.0.1`을 사용한다.
- `0`번: 예약되어있으나 특별한 용도는 지정되어있지 않다.

### 클래스 기반 IP 주소 체계의 문제

1. 지원할 수 있는 호스트의 개수가 제한되어있어 비효율적이다. 예를 들어 호스트 개수가 300개인 네트워크는 최대 254개의 호스트를 지원하는 클래스 C를 사용할 수 없으므로 클래스 B를 사용해야한다. 그러나 클래스 B는 약 6만 5000개의 호스트를 지원하기 때문에 IP 주소 공간이 낭비된다.
2. 네트워크를 결합하여 슈퍼넷(supernet; 유사한 네트워크 접두사를 가진 서브넷의 집합)을 만들 수 없다. 예를 들어 클래스 C 네트워크에 속하는 `192.168.1.0`과 `192.168.0.0`을 결합하고자 한다. 그러나 클래스 C의 서브넷 마스크가 `255.255.255.0`으로 고정되어있어 `192.168.1`과 `192.168.0`은 서로 다른 네트워크로 구분된다.

### CIDR 표기법

**CIDR(Classless Inter-Domain Routing notation)**은 IP 주소를 서브넷 마스크와 결합하여 표시하는 방법이다.

```
IP주소/서브넷마스크비트
```

이렇게 표현하면 유연하게 네트워크와 호스트 식별자를 결정할 수 있어 낭비를 줄일 수 있다. 또한 기존에는 불가능했던 슈퍼넷을 생성할 수 있다.

### 공인 IP와 사설 IP

**공인 IP(Public IP)**는 네트워크 외부와 통신할 때 사용하는 주소이다. ISP(Internet Service Provider)가 제공하며 전세계에서 유일하다. **사설 IP(Private IP)**는 네트워크 내부에서만 사용하는 주소이다. 네트워크 관리자나 라우터가 제공하며 해당 네트워크 내에서만 유일하다. 네트워크에 속한 여러 호스트는 해당 네트워크 내에서 고유한 private ip를 가지고 하나의 public ip를 공유해 인터넷에 접속할 수 있게 된다.

사설 IP는 대역이 고정되어있다. A 클래스에서는 `10.xxx.xxx.xxx`, B 클래스에서는 `172.10.xxx.xxx`, C 클래스에서는 `192.168.xxx.xxx`를 사용한다.

### 고정 IP와 유동 IP

고정 IP(Static IP)는 호스트에 고정적으로 부여되는 주소로 반납하기 전까지 독점한다. 보안성이 우수하다. 유동 IP(Dynamic IP)는 호스트에게 임시로 부여되는 주소로, 대부분 호스트는 네트워크에 접속할 때마다 새로운 IP를 발급받는다(컴퓨터가 부팅하면 DHCP 프로토콜을 사용하여 DHCP 서버나 라우터로부터 IP 주소를 할당받는다).
...
혼잡은 기본적으로 네트워크가 처리할 수 있는 패킷보다 많은 패킷이 입력되어 발생한다. 패킷이 분실되는 경우, 혹은 패킷 순서가 바뀌어 패킷을 버리는 경우 송신 호스트는 타임아웃 동작을 통해 패킷을 재전송한다. 결국 네트워크 혼잡도가 계속 증가하는 악순환에 빠지게 된다. 혼잡이 포착되면 타임아웃 시간을 늘려 재전송이 빈번하게 일어나지 않도록 하거나 트래픽이 적은 경로로 패킷을 전송하여 혼잡도를 줄일 수 있다. 한편 패킷이 네트워크에서 생존할 수 있는 시간을 너무 크게 주면 불필요한 부하를 줄 수 있고, 너무 작게 주면 패킷이 혼잡 제어제로 제거되어 재전송이 발생할 수 있다. 따라서 패킷의 생존 시간을 적절히 설정해주어야 한다.
...
TODO: 왜 DHCP_OFFER는 브로드캐스팅일까?

브로드캐스팅인지 유니캐스팅인지 확실하지 않다.



> ARP/RARP는 데이터 링크 계층의 프로토콜이다. ARP(Address Resolution Protocol)는 IP 주소를 MAC 주소로 변환하는 프로토콜이다. 송신 호스트가 수신 호스트의 MAC 주소를 얻기 위해 사용한다. RARP(Reverse Address Resolution Protocol)는 MAC 주소를 IP 주소로 변환하는 프로토콜이다. IP 주소는 파일에 보관되는데, 파일 시스템이 존재하지 않는 하드웨어의 경우 LAN 카드의 MAC 주소만 알기 때문에 사용한다.
> ICMP는 네트워크 계층의 프로토콜이다. ICMP(Internet Control Message Protocol)는 전송 오류가 발생하면 복구 작업을 위해 오류 메시지를 전송하는 프로토콜이다.
...
- **LAN이란?** **LAN(Local Area Network)**는 한정된 지역(동일한 건물-집, 회사, 학교 등) 내에서 연결된 컴퓨터나 장치들의 네트워크이다.
- **LAN 카드란?** LAN 카드(LAN card)는 네트워크 어댑터(Network adapter)나 이더넷 어댑터(Ethernet adapter)라고 하며 LAN이나 WLAN에 물리적으로 연결될 수 있게 하는 하드웨어이다. LAN 카드는 네트워크 상에서 LAN 카드를 식별하는 고유한 주소인 MAC(Media Access control) 주소를 가진다. (MAC 주소는 이론적으로는 전세계적으로 고유하나 제조 기업에 의해 관리되므로 제조 기업이 다르면 MAC 주소가 중복될 수 있다. ARP 프로토콜과 IEEE 명세 등을 사용하여 MAC 주소 충돌을 해결할 수 있다.)
- **이더넷(Etherent)**이란? LAN에서 연결된 호스트들이 데이터를 주고받을 때 사용하는 프로토콜이다.
- **Wi-Fi(Wiress Fidelity)란?** WLAN(Wireless LAN)에 연결된 호스트들이 데이터를 주고받을 때 사용하는 프로토콜이다.
...
Ethernet adapter Local Area Connection
  Ipv4 Address: 컴퓨터가 라우터로부터 할당받은 private IP 주소
  Subnet Mask: 컴퓨터가 연결된 네트워크 대역폭
  Default Gateway: 라우터(공유기)의 private IP 주소
```

- Ethernet adapter는 LAN 카드(NIC; Network Interface Controller)를 뜻하며, Local Area는 LAN(Local Area Network)을 뜻한다.

- IP 주소와 서브넷 마스크, 기본 게이트웨이 주소는 호스트가 네트워크에 연결되기 위해 반드시 필요한 요소이다.
- 기본 게이트웨이란? 기본 게이트웨이(Default Gateway)는 호스트를 외부 네트워크(특히 인터넷)에 연결하는 하드웨이다. 공유기나 라우터가 여기 속한다.
- 공유기를 사용한다면 DNS 서버와 DHCP 서버의 IP 주소는 자동으로 공유기의 IP 주소로 설정된다. 

공유기를 사용하여 네트워크에 접속했다면 대개 서브넷 마스크는 `255.255.255.0`이고 IPv4 주소와 기본 게이트웨이가 `192.168.xxx`로 시작할 것이다. 이는 공유기가 주로 C 클래스 대역을 사용하고, C 클래스에서 사용하는 사설 IP 주소는 `192.168.xxx.xxx`이기 때문이다. 이때 공유기는 주로 마지막 옵텟으로 1을 사용하여 게이트웨이 주소가 `192.168.xxx.1`인 경우가 많을 것이다. 공유기에 연결된 모든 디바이스들은 LAN을 이룬다고 할 수 있다.
...
- [TCP/IP 이론 — IP 주소, 서브넷 마스크 그리고 기본 게이트웨이](https://medium.com/pocs/tcp-ip-%EC%9D%B4%EB%A1%A0-ip-%EC%A3%BC%EC%86%8C-%EC%84%9C%EB%B8%8C%EB%84%B7-%EB%A7%88%EC%8A%A4%ED%81%AC-%EA%B7%B8%EB%A6%AC%EA%B3%A0-%EA%B8%B0%EB%B3%B8-%EA%B2%8C%EC%9D%B4%ED%8A%B8%EC%9B%A8%EC%9D%B4-ccd6d832711e)
- http://www.incodom.kr/Gateway
- 쉽게 배우는 데이터 통신과 컴퓨터 네트워크 Chapter 03 네트워크 기술
- 쉽게 배우는 데이터 통신과 컴퓨터 네트워크 Chapter 08 네트워크 계층
- 쉽게 배우는 데이터 통신과 컴퓨터 네트워크 Chapter 07 IP 프로토콜
...
- IPv4에서 주소 고갈 문제는 어떻게 해결하고 있나요?
  ...
  - private IP를 사용해서 얻을 수 있는 이점이 있나요?
...
why use ip address instead of mac
