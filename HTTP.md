...
**HTTP(HyperText Transfer Protocol)**는 웹(W3) 상에서 클라이언트와 서버가 자원을 주고 받을 때 사용하는 통신 규약이다. 기본 포트는 80이다. 클라이언트는 서버에게 HTTP 요청을 하고, 서버는 이 요청에 대하여 클라이언트에게 HTTP 응답을 한다.
...
`Keep-Alive` 커넥션, 가상 호스팅, 프록시 연결 등을 지원한다.
...
TCP는 신뢰성 있는 데이터를 보장하므로 데이터를 순서에 맞게 보내야한다. 따라서 앞 선 패킷에 대한 처리가 끝날 때까지 뒤에 오는 패킷의 처리가 대기하는 **HOL(Head Of Line) Blocking** 문제가 발생한다(파이프라이닝으로 클라이언트 요청의 대기는 회피할 순 있지만 서버는 응답의 대기를 회피할 수 없다). 또한 지속 커넥션에서 요청의 헤더가 중복되는 경우가 많아 대역폭이 낭비되는 문제가 있었다.
...
소켓(socket)은 운영체제 내부에 시스템 콜의 형태로 구현된 `인터페이스`로 응용 프로그램에게 TCP/UDP 통신 기능을 제공한다. 종단점으로서 포트(port)라고 하기도 한다. WebSocket은 하나의 TCP 커넥션으로 실시간 양방향 통신을 제공하는 `전송 계층 프로토콜`이다.
